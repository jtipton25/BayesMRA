...
author:
  - name: John Tipton
    ...
    email: \email{jrtipton@uark.edu}
    url: http://www.johnrtipton.com
  - name: Jean Remy
    affiliation: University of Arkansas
    address: >
      309 SCEN
  ...
  - name: Thuy Scanlon
    ...
    address: >
      ...
      University of Arkansas
      Fayetteville, AR 72701
    email: \email{tvscanlo@uark.edu}
    url:
title:
  formatted: "Bayesian Multi-resolution Spatial Regression: Fitting large spatial models using \\pkg{BayesMRA}"
  # If you use tex in the formatted title, also supply version without
  plain:     "Bayesian Multi-resolution Spatial Regression: Fitting large spatial models using BayesMRA"
  # For running headers, if needed
  short:     "\\pkg{BayesMRA}: Bayesian Multi-resolution Spatial Regression"
abstract: >
...
```{r setup, echo = FALSE}
knitr::opts_chunk$set(fig.width = 16, fig.height = 9, dpi = 300, dev.args = list(type = "cairo"), dev = 'png', fig.align = 'center')
```

# Todos

- Thuy: edit the file  scripts/mra-maunscript.R to get better figure formats. Proofread the spelling and text size. See [https://ggplot2.tidyverse.org/reference/ggtheme.html](https://ggplot2.tidyverse.org/reference/ggtheme.html) for some example themes that make it easy to change all font sizes simultaneously.

# Outline and writing goals

- Thuy: Write up 2-3 paragraphs about the importance of effective sample size per second.

- Thuy: Write out the model statement and priors. Follow Nychka et. al. Multiresolution where applicable and use your best judgement for the priors on $\mathbf{\tau}^2$ and $\mathbf{\beta}$. From the model statement, write out the posterior distribution.


- Jean Remy: work out the full conditionals for the updates of $\boldsymbol{\beta}$, $\boldsymbol{\alpha}$, $\mathbf{\tau}^2$ and $\sigma^2$

- Jean Remy: Focus on the writing of the spatial models in the ** Defining the MRA model** section. Pay attention to how Nychka et. al. Multiresolution... writes the model statement. 

- John: work on package code and fit heaton et. al. data [https://github.com/finnlindgren/heatoncomparison](https://github.com/finnlindgren/heatoncomparison)

- John: write up sum-to-zero constraint and multi-resolution factorization for better scaling to massive data

# Introduction

Fitting spatial models in a Bayesian framework is computationally challenging because of the curse of dimensionality where the computational cost for fitting full-rank spatial models scales with $O(n^3)$. Instead, a number of reduced rank spatial approximations have been developed **citations**. There is a variety of software in `R` for fitting reduced-rank spatial models in the frequentist paradigm, including **list packages** and for fitting reduced-rank models in the Bayesian paradigm **list packages**. This work provides a set of tools in `R` for efficiently fitting a multiresolution basis function spatial model similar to @nychka2015multiresolution. The goal of this work is twofold; first, to provide software for fitting the multiresolution approximation using Markov Chain Monte Carlo; second, to make available a set of functions for developing the multiresolution process for other developers wishing to fit spatial and spatio-temporal models in `R` using the multiresolution approach.
...
**simulated data code**
```{r, simulate_data, eval = FALSE, echo = FALSE}
set.seed(11)
...
```{r, out.width = "49%", echo = FALSE, fig.align = "center", fig.show = "hold", fig.cap = "Sparsity of MRA precision matrices"}
knitr::include_graphics(here::here("images", "precision-matrix-sparsity.png"))
```
...
```{r, out.width = "49%", echo = FALSE, fig.align = "center", fig.show = "hold"}
knitr::include_graphics(here::here("images", c("MRA-grid.png", "MRA-grid-neighbors.png")))
```


<!-- ```{r MRA-grid, cache = TRUE, fig.height = 4.5, fig.width = 4.5, fig.cap = "MRA gridpoints"} -->
<!-- data.frame( -->
...
<!-- ```{r plot-neighbors, cache = TRUE, warning = FALSE} -->
...
```{r, out.width = "49%", echo = FALSE, fig.align = "center", fig.show = "hold"}
knitr::include_graphics(here::here("images", c("MRA-random-effects.png", "MRA-simulated-data.png")))
```

<!-- ```{r plot-random-effects, cache = TRUE, warning = FALSE} -->
...
<!-- ```{r plot-data-process, cache = TRUE} -->
<!-- zlims <- c( -->
...
```{r, out.width = "100%", echo = FALSE, fig.align = "center", fig.show = "hold"}
knitr::include_graphics(here::here("images", "MRA-resolutions.png"))
```


<!-- ```{r plot-multiresolution-process, cache = TRUE} -->
<!-- ## MRA spatial process -->
...
## Fitting the MRA model to simulated data
...
**subsample data code**
```{r, sample_data, eval = FALSE, echo = FALSE}
set.seed(111)
n_sites   <- 500
s         <- sample(N, n_sites)
y_s       <- y[s, ]
...
```{r, out.width = "100%", echo = FALSE, fig.align = "center", fig.show = "hold"}
knitr::include_graphics(here::here("images", "sample-data.png"))
```


<!-- ```{r plot-sampled-data, cache = TRUE, fig.height = 4.5, fig.width = 4.5} -->
<!-- data.frame( -->
<!--   x     = locs_s[, 1], -->
<!--   y     = locs_s[, 2], -->
...
**fit model code**
```{r params-MCMC, eval = FALSE, echo = FALSE}
params <- list(
  n_mcmc    = 500,
  n_adapt   = 500,
  n_thin    = 1,
...
**fitting the model**
```{r fit-model, eval = FALSE, echo = FALSE}
set.seed(999)
...
```{r, out.width = "100%", echo = FALSE, fig.align = "center", fig.show = "hold"}
knitr::include_graphics(here::here("images", c("trace-plots.png", "trace-plots-alpha.png")))
```



<!-- ```{r trace-plots, cache = TRUE} -->
...
<!-- ```{r trace-plots-alpha, cache = TRUE} -->
...
```{r, out.width = "100%", echo = FALSE, fig.align = "center", fig.show = "hold"}
knitr::include_graphics(here::here("images", "estimated-vs-predicted.png"))
```



<!-- ```{r estimated-vs-predicted, cache = TRUE} -->
...
```{r, out.width = "49%", echo = FALSE, fig.align = "center", fig.show = "hold"}
knitr::include_graphics(here::here("images", c("MRA-random-effects.png", "fitted-spatial-random-effects.png")))
```

<!-- ```{r estimated-alphas, cache = TRUE, warning = FALSE} -->
...
```{r, out.width = "49%", echo = FALSE, fig.align = "center", fig.show = "hold"}
knitr::include_graphics(here::here("images", c("MRA-resolutions.png", "fitted-process.png")))
```


<!-- ```{r simulated-vs-estimated-process, cache = TRUE, fig.cap = "Simulated and estimated MRA processes"}  -->

<!-- ## Estimated MRA spatial process -->
...
```{r, out.width = "49%", echo = FALSE, fig.align = "center", fig.show = "hold"}
knitr::include_graphics(here::here("images", "sim-vs-fitted-process.png"))
```



<!-- ```{r plot-sim-vs-fitted-effects, cache = TRUE} -->
...
# A real data example
* Use data of Heaton et. al.
* Calculate the effective sample size per second
