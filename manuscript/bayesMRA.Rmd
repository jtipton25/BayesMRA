...
Directly specifying a functional form for the covariance function $cov(\mathbf{s}_i, \mathbf{s}_j)$ is commonly done in the geostatistical literature; however, there are many drawbacks to specifying 
...
```{r plot-precision-matrices, warning = FALSE, cache = TRUE, dev.args = list(pointsize = 4), fig.cap = "Plot showing the locations of nonzero elements of the precision matrix at each resolution."}
...
```{r, sample_data, dependson = "simulate_data"}
set.seed(111)
n_sites   <- 500
s         <- sample(N, n_sites)
y_s       <- y[s, ]
...
```{r, out.width = "49%", echo = FALSE, fig.align = "center", fig.show = "hold"}
knitr::include_graphics(here::here("images", "fitted-process.png"))
```
