...
Fitting spatial models in a Bayesian framework is computationally challenging because of the curse of dimensionality. 
...
```{r plot-precision-matrices, warning = FALSE, cache = TRUE, dependson = "simulate_data", dev.args = list(pointsize = 4), fig.cap = "Plot showing the locations of nonzero elements of the precision matrix at each resolution."}
...
```{r MRA-grid, cache = TRUE, dependson = "simulate_data", fig.height = 4.5, fig.width = 4.5, fig.cap = "MRA gridpoints"}
data.frame(
...
```{r plot-neighbors, cache = TRUE, dependson = "simulate_data", warning = FALSE}
...
```{r plot-random-effects, cache = TRUE, dependson = "simulate_data", warning = FALSE}
...
```{r plot-data-process, cache = TRUE, dependson = "simulate_data"}
zlims <- c(
...
```{r plot-multiresolution-process, cache = TRUE, dependson = "simulate_data"}
## MRA spatial process
...
```{r plot-sampled-data, cache = TRUE, dependson = "simulate_data", fig.height = 4.5, fig.width = 4.5}
data.frame(
  x     = locs_s[, 1],
  y     = locs_s[, 2],
...
```{r fit-model, cache = TRUE, dependson = "simulate_data"}
set.seed(999)
...
```{r trace-plots, cache = TRUE, dependson = c("simulate_data", "fit-model")}
...
```{r trace-plots-alpha, cache = TRUE, dependson = c("simulate_data", "fit-model")}
...
```{r estimated-vs-predicted, cache = TRUE, dependson = c("simulate_data", "fit-model")}
...
```{r estimated-alphas, cache = TRUE, dependson = c("simulate_data", "fit-model"), warning = FALSE}
...
```{r simulated-vs-estimated-process, cache = TRUE, dependson = c("simulate_data", "fit-model"), fig.cap = "Simulated and estimated MRA processes"} 
...
```{r plot-sim-vs-fitted-effects, cache = TRUE, dependson = c("simulate_data", "fit-model")}
