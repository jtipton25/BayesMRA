...
dat_alpha <- data.frame(
  alpha = c(out$alpha),
  iteration = rep(1:nrow(out$alpha), times = ncol(out$alpha)),
  par_id    = rep(1:ncol(out$alpha), each = nrow(out$alpha)))

dat_levels <- data.frame(
  par_id = 1:ncol(out$alpha)  ,
  knot      = factor(unlist(sapply(1:M, function(m) 1:n_dims[m]))),
  resolution = factor(paste("resolution", dims_idx))
)

dat_alpha <- merge(dat_alpha, dat_levels)
...
# simulated MRA process
W_alpha_res_sim <- unlist(sapply(1:M, function(m) W[, dims_idx == m] %*% alpha[dims_idx == m], simplify = "matrix"))
dimnames(W_alpha_res_sim) <- list(
  site = 1:N, 
  res  = paste("resolution", 1:M)
)

## Estimated MRA spatial proces
W_pred <- mra_wendland_2d_pred(locs, out$MRA)$W_pred
W_alpha_res_fit <- unlist(sapply(1:M, function(m) W_pred[, out$MRA$dims_idx == m] %*% apply(out$alpha, 2, mean)[out$MRA$dims_idx == m], simplify = "matrix"))
dimnames(W_alpha_res_fit) <- list(
  site = 1:N, 
  res  = paste("resolution", 1:M)
)
dat_locs <- data.frame(
...
zlims <- range(c(
  apply(W_alpha_res_sim, 1, sum), 
  apply(W_alpha_res_fit, 1, sum)))


sim_MRA <- rbind(
  merge(dat_locs, as.data.frame.table(W_alpha_res_sim, responseName = "W_alpha")),
  data.frame(  
    site    = 1:N,
    lat     = locs[, 2],
    lon     = locs[, 1],
    W_alpha = apply(W_alpha_res_sim, 1, sum),
    res     = "full process"
  )
) %>%
  ...
  xlab("Longitude") + 
  ylab("Latitude") + 
  ggtitle("Simulated Multiresolution process") +
  colorspace::scale_fill_continuous_diverging("Blue-Red 3", limits = zlims) +
  theme_bw() +
  theme_custom(0.5)

estimated_MRA <- rbind(
  merge(dat_locs, as.data.frame.table(W_alpha_res_fit, responseName = "W_alpha")),
  data.frame(  
    site = 1:N,
    lat  = locs[, 2],
    lon  = locs[, 1],
    W_alpha = apply(W_alpha_res_fit, 1, sum),
    res = "full process"
  )
) %>%
  ggplot(aes(x = lon, y = lat, fill = W_alpha)) +
