...
W_fit <- do.call(cbind, out$MRA$W)
tW_fit <- t(W_fit)
tWW_fit <- tW_fit %*% W_fit
n_dims_fit   <- rep(NA, length(out$MRA$W))
dims_idx_fit <- NULL
for (i in 1:M) {
  n_dims_fit[i] <- ncol(out$MRA$W[[i]])
  dims_idx_fit  <- c(dims_idx_fit, rep(i, n_dims_fit[i]))
}
...
for (k in 1:nrow(out$beta)) {
  ...
  alpha_post[k, ]   <- as.vector(rmvnorm.canonical.const(n = 1, mu = b, Q = A, A = t(A_constraint), a = a_constraint, Rstruct = A_chol))  
}
