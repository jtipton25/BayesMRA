...
* Recovering $\boldsymbol{\alpha}$ using composition sampling

```{r trace-plots-alpha, cache = TRUE, cache.extra = tools::md5sum(here::here("results", "fit-mra-sim.RData")), fig.align = 'center'}

##
## composition sampling to extract alpha
##

W_fit        <- out$MRA$W
tW_fit       <- t(W_fit)
tWW_fit      <- tW_fit %*% W_fit
n_dims_fit   <- out$MRA$n_dims
dims_idx_fit <- out$MRA$dims_idx


alpha_post <- matrix(0, nrow(out$beta), ncol(W_fit))
## set up a linear constraint so that each resolution sums to one
A_constraint <- sapply(1:out$MRA$M, function(i){
        tmp = rep(0, sum(n_dims_fit))
        tmp[dims_idx_fit == i] <- 1
        return(tmp)
    })

a_constraint <- rep(0, M)
Q_alpha_fit <- make_Q_alpha_2d(sqrt(n_dims_fit), rep(1, out$MRA$M))

Q_alpha_tau2_fit <- make_Q_alpha_tau2(Q_alpha_fit, out$tau2[1, ])
A      <- tWW_fit / out$sigma2[1] + Q_alpha_tau2_fit
A_chol <- chol(A)
b                <- as.vector(tW_fit %*% (y_s - X_s %*% out$beta[1, ]))
  
tmp <- backsolve(A_chol, forwardsolve(A_chol, b) + rnorm(nrow(A_chol)))


for (k in 1:nrow(out$beta)) {
  if (k %% 10 == 0) {
    message("On iteration ", k, " out of ", nrow(out$beta))
  }
  Q_alpha_tau2_fit <- make_Q_alpha_tau2(Q_alpha_fit, out$tau2[k, ])
  A                <- tWW_fit / out$sigma2[k] + Q_alpha_tau2_fit
  b                <- as.vector(tW_fit %*% (y_s - X_s %*% out$beta[k, ]))
  alpha_post[k, ]  <- as.vector(rmvnorm.canonical.const(n = 1, b = b, Q = A, A = t(A_constraint), a = a_constraint, Rstruct = A_chol))  
}

## add in posterior draws for alpha to model fit
